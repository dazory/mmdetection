...
@PIPELINES.register_module()
class MultiViewDefaultFormatBundle:
    """Default formatting bundle.
...
@PIPELINES.register_module()
class MultiViewCollect:
    def __init__(self,
                 keys=None,
                 fields=None,
                 fields_with_keys=None,
                 meta_keys=('filename', 'ori_filename', 'ori_shape',
                            'img_shape', 'pad_shape', 'scale_factor', 'flip',
                            'flip_direction', 'img_norm_cfg')):
        self.keys = keys if keys is not None else []
        self.fields = fields if fields is not None else []
        self.fields_with_keys = fields_with_keys if fields_with_keys is not None else {}
        self.meta_keys = meta_keys
    ...
    def __call__(self, results):
        ...
        data['img_metas'] = DC(img_meta, cpu_only=True)
        for field in self.fields:
            for key in results.get(f'{field}_fields', []):
                data[key] = results[key]
        for key in self.keys:
            data[key] = results[key]
        for field, key in self.fields_with_keys.items():
            data_list = []
            for _key in results.get(f'{field}_fields', []):
                data_list.append(results[_key])
            data[key] = data_list
        return data
