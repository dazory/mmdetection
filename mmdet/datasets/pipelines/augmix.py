...
def posterize(pil_img, level, use_random=True, **kwargs):
    level = int_parameter(sample_level(level) if use_random else level, 4)
...
def solarize(pil_img, level, use_random=True, **kwargs):
  level = int_parameter(sample_level(level) if use_random else level, 256)
...
class AugMix:
    def __init__(self,
                 aug_list='augmentations', no_jsd=False, aug_severity=1,
                 num_views=3):
