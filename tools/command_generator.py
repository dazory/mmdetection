...
def get_command(type, seed, category, config_name):
    ...
    if type == 'train':
        comm = f'python3 -u /ws/external/tools/train.py ' \
               f'--seed {seed} ' \
               f'--deterministic /ws/external/configs/_dshong/cityscapes/{category}/{config_name}.py ' \
               f'--work-dir /ws/data/dshong/_dshong/cityscapes/{category}/{folder_name}'
    elif type == 'test':
        comm = f"python3 -u /ws/external/tools/analysis_tools/test_robustness.py " \
               f"/ws/data/dshong/_dshong/cityscapes/{category}/{folder_name}/{config_name}.py " \
               f"/ws/data/dshong/_dshong/cityscapes/{category}/{folder_name}/epoch_2.pth " \
               f"--out /ws/data/dshong/_dshong/cityscapes/{category}/{folder_name}/epoch_2.pkl " \
               f"--corruptions benchmark --eval bbox --load-dataset corrupted " \
               f"| tee /ws/data/dshong/_dshong/cityscapes/{category}/{folder_name}/test_robustness_result_2epoch.txt"
    elif type == 'parse':
        comm = f"python3 /ws/external/tools/analysis_tools/parse_txt2dict.py " \
               f"/ws/data/dshong/_dshong/cityscapes/{category}/{folder_name}/test_robustness_result_2epoch.txt " \
               f"/ws/data/dshong/_dshong/cityscapes/{category}/{folder_name}/{config_name}.py " \
               f"| tee /ws/data/dshong/_dshong/cityscapes/{category}/{folder_name}/summary_result_2epoch.txt"
    elif type == 'all':
